...
-   Non-default linters that we considered but dismissed due to annoyance or disagreement include:

    -   `library_call_linter` (not relevant in the context of R package development)
    ...
    -   `pipe_consistency_linter` (although we generally prefer base R's pipe operator over magrittr's, we still like to be able to use the latter for specific
        cases that the former doesn't support like referencing the input placeholder multiple times or piping input to magrittr's pipe assignement operator
        `%<>%`)
