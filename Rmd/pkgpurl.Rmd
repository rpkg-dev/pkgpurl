...
-   Figure out why running `process_pkg()` for the *pal* pkg always produces the following error after changing R code (semantic code changes landing in the
    ...
    NOTE that an additional `pkgload::unload()` call before `devtools::install()` doesn't seem to help...
...
#' 4. Paragraph(s) below headings named *DESCRIPTION* (case-insensitive, but without any inline formatting), that in turn are hierarchically below a title or
#'    subtitle heading, are be used as descriptions for the (sub)titles. This maps to the `desc` key of the reference index' YAML.
#'
#' # Parsing example
#'
