...
-   Figure out why running `process_pkg()` for the *pal* pkg always produces the following error after changing R code (semantic code changes landing in the
    `R/pal.gen.R` file, excl. documentation/comments/formatting like whitespaces):

    ``` error
    Error in purl_rmd(path = path, add_copyright_notice = add_copyright_notice,  : 
      lazy-load database '/home/salim/.by_pkg_mngr/r/pal/R/pal.rdb' is corrupt
    In addition: Warning messages:
    1: In purl_rmd(path = path, add_copyright_notice = add_copyright_notice,  :
      restarting interrupted promise evaluation
    2: In purl_rmd(path = path, add_copyright_notice = add_copyright_notice,  :
      internal error -3 in R_decompress1
    ```

    Seems related to [this SO answer](https://stackoverflow.com/a/10374507/7196903).

    This problem doesn't occur when (re-)processing pkg *pkgpurl*, so some additional complexities in pal must be causing it... simply adding another restart of
    R after building package doc won't work because it interrupts `process_pkg()` (restarting R is only possible at the end of the fn) 😕

-   Figure out if (and how) it's possible to print `process_pkg()`'s output to RStudio's *Build* pane instead of the console. I couldn't find anything related
    in the [rstudioapi reference](https://rstudio.github.io/rstudioapi/reference/index.html). Maybe ask in the [*package development* section on
    `community.rstudio.com`](https://community.rstudio.com/c/package-development/11)?

NOTES:

-   Without restarting the R session (`restart_r_session = FALSE`) when `build_and_install = TRUE`, the following error occurs after running
    `process_pkg(build_and_install = TRUE, restart_r_session = FALSE)` *twice*:

    ``` error
    Error in fetch(key) : 
      lazy-load database '/home/salim/.by_pkg_mngr/r/pkgpurl/help/pkgpurl.rdb' is corrupt
    ```

    Cf. <https://github.com/r-lib/pkgdown/issues/892#issuecomment-439894975>
...
process_pkg <- function(path = ".",
                        ...
                        gen_pkgdown_ref = getOption("pkgpurl.gen_pkgdown_ref",
                                                    default = TRUE),
                        document = TRUE,
...
#'   other information is extracted from the package's `DESCRIPTION` file. A logical scalar or `NULL`. If `NULL`, the notice will be added only if `path` [is
#'   actually an R package directory][pal::is_pkg_dir]. Defaults to the R option `pkgpurl.add_copyright_notice`.
...
#'   licenses](https://www.gnu.org/licenses/gpl-howto.html). The license is determined from the package's `DESCRIPTION` file and currently only the
#'   [`AGPL-3.0-or-later` license](https://spdx.org/licenses/AGPL-3.0-or-later.html) is supported. A logical scalar or `NULL`. If `NULL`, it will be added only
#'   if `path` [is actually an R package directory][pal::is_pkg_dir]. Defaults to the R option `pkgpurl.add_license_notice`.
...
-   Exclude headings named `EXPORTED` for (sub)title determination.

-   Figure out why the example throws the following error during testing:

    ``` error
    Error: C stack usage  7972020 is too close to the limit
    ```

    No error occurs when the example is run interactively or via `Rscript`. Maybe it has to do with C++ pointers used in pkg xml2?
...
#'
#' Basically, all of the pkgdown reference index except `desc` keys are derived from `rmd`'s [Markdown **headings**](https://pandoc.org/MANUAL.html#headings)
#' and their **hierarchy**. The `desc` keys, however, are assembled from the text paragraphs below some specially named headings (see below).
#'
#' To be able to unambiguously map an `.Rmd` file's heading hierarchy to the pkgdown reference index, some rules are necessary. Attention must be paid to the
#' fact that, while (R) Markdown supports up to six heading levels (corresponding to HTML's `<h1>`–`<h6>` tags), the pkgdown reference index only supports up
#' to two (`title` and `subtitle`).
#' 
...
#'    topic heading is ignored. This maps to the `title` and `subtitle` keys of the reference index' YAML.
#' 4. Text paragraph(s) below headings named *DESCRIPTION* (case-insensitive, but without any inline formatting), that in turn are hierarchically below a title
#'    or subtitle heading, will be used as descriptions for the (sub)titles. This maps to the `desc` key of the reference index' YAML.
#'
#' When fed to `gen_pkgdown_ref()`, the following simplified example `.Rmd`...
#'
...
#' ```
#'
#' ...yields this pkgdown index YAML:
#'
  ...
  hierarchy <-
    ...
    dplyr::left_join(y = tibble::tibble(i = .$i[.$is_heading],
                                        ...
                                        & rmd_xml[i] %>%
                                          ...
                                          stringr::str_detect(pattern = "^(?i)description$"),
                                        is_ignored =
                                          xml2::xml_length(rmd_xml[i]) == 1L
                                        & rmd_xml[i] %>%
                                          xml2::xml_child(search = "d1:text") %>%
                                          xml2::xml_text(trim = TRUE) %>%
                                          stringr::str_detect(pattern = "^(?i)(internal|temp(orary)?)$")),
                     by = "i") %>%
    ...
    result <-
      ...
      dplyr::group_map(function(data_title, key) {
        ...
        items_subtitle <-
          ...
          purrr::when(any(is.na(data_title$subtitle)) ~ .[c(length(.), seq_len(length(.) - 1L))],
                      ~ .)
      ...
      purrr::when(any(is.na(data_ref_i$title)) ~ .[c(length(.), seq_len(length(.) - 1L))],
                  ~ .)
  }
