...
-   [ ] Support more OSS licenses in `add_license_notice`. Default license headers would have to be systematically fetched from somewhere for this!
    ...
    A viable solution could be to feed the license info in the pkg DESCRIPTION to the OSI API, query the SPDX identifier and based on that scrape the SPDX page
    for the *Standard License Header* section. But the OSI API doesn't seem to offer any (fuzzy) search endpoint... so at least we need to implement a mapping
    from the [license identifiers officially supported by R](https://cran.rstudio.com/doc/manuals/r-devel/R-exts.html#Licensing) (like the "standard" short
