...
- Setting up a new project to write an R package in the R Markdown format involves some additional steps compared to the classic approach. A suitable
  convenience function like `create_rmd_package()` to set up all the necessary parts could probably be added to [usethis](usethis.r-lib.org/) in the future.
...
- Other than in `.R` files, RStudio currently doesn't support auto-completion of [roxygen2 tags](https://roxygen2.r-lib.org/articles/rd.html) in `.Rmd` files.
  The same applies to the roxygen2 comment continuation when inserting a newline. This is a [known issue](https://github.com/rstudio/rstudio/issues/5809) and
