...
-   **Easily toggle code inclusion**
    ...
    can (and should) still use `usethis::use_data(...)` (optionally with `overwrite = TRUE`) to generate the files under `data/` holding external package data
    as well as the `R/sysdata.rda` file (using `internal = TRUE`) holding internal package data.
...
-   **Additional workflow step**
    ...
    For the time being, it's recommended to set up a custom shortcut[^custom-shortcut] for one or both of `pkgpurl::purl_rmd()` and `pkgpurl::process_pkg()`
    which are registered as [RStudio add-ins](https://rstudio.github.io/rstudioaddins/).
...
-   **Missing roxygen2 auto-completion**
    ...
    These are [known issues](https://github.com/rstudio/rstudio/issues/5809)
