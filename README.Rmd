...
[`pkgpurl::purl_rmd()`](https://rpkg.dev/pkgpurl/reference/purl_rmd.html) is just a less cumbersome alternative to the Makefile approach outlined by him.
...
-   **Mix Markdown and Code**
    ...
    syntax](https://pandoc.org/MANUAL.html#pandocs-markdown) like inline formatting, lists, tables, quotations or math[^any-doc-syntax].
    ...
    [canonicalization](https://en.wikipedia.org/wiki/Canonicalization) (on file save[^canon]). For example, it allows paragraphs being wrapped automatically at
    the desired line width; or to write a minimal sloppy [pipe table](https://pandoc.org/MANUAL.html#extension-pipe_tables) that is automatically normalized to
    a beautifully formatted and actually readable one.
...
-   **All your code in a single, well-structured file**
    ...
    multiple files. The popular (and very useful) book *R Packages* [gives the following advice](https://r-pkgs.org/r.html#code-organising):
    ...
    described here](https://rpkg.dev/pkgpurl/reference/gen_pkgdown_ref.html#details), which even allows the [pkgdown `Reference:`
    ...
    you re-organize (and thus most likely improve) your package's code structure whenever you intend to improve the pkgdown reference – and vice versa.
    ...
    suffix](https://rpkg.dev/pkgpurl/reference/purl_rmd.html#-rmd-files-excluded-from-purling).
...
-   **Improved overview and navigation**
    ...
    standard](https://support.rstudio.com/hc/en-us/articles/200484568-Code-Folding-and-Sections) of `.R` files. It makes it easy to find your way around source
...
-   **Easily toggle code inclusion**
    ...
    well as [internal data](https://r-pkgs.org/data.html#data-sysdata). This avoids having to outsource the code to separate files under `data-raw/` and adding
    the directory to `.Rbuildignore`, i.e. no need to use `usethis::use_data_raw()`. Instead, you just set `purl = FALSE` for the relevant code chunk(s). You
    can (and should) still use [`usethis::use_data()`](https://usethis.r-lib.org/reference/use_data.html) (optionally with `overwrite = TRUE`) to generate the
    files under `data/` holding external package data as well as the `R/sysdata.rda` file (using `internal = TRUE`) holding internal package data.
...
-   **Additional workflow step**
    ...
    workflows: Running [`pkgpurl::purl_rmd()`](https://rpkg.dev/pkgpurl/reference/purl_rmd.html) to generate the `R/*.gen.R` files from the original `Rmd/*.Rmd`
    ...
    For the time being, it's recommended to set up a custom shortcut[^custom-shortcut] for one or both of
    [`pkgpurl::purl_rmd()`](https://rpkg.dev/pkgpurl/reference/purl_rmd.html) and
    [`pkgpurl::process_pkg()`](https://rpkg.dev/pkgpurl/reference/process_pkg.html) which are registered as [RStudio
...
[^any-doc-syntax]: Actually, you could write anything you like in any syntax outside of R code chunks as long as you don't mind the file to be *knittable*
(which it doesn't have to be).

[^canon]: It basically sends the (R) Markdown file on a "Pandoc round trip" on every file save.

[^custom-shortcut]: I personally recommend to use the shortcut <kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>V</kbd> since it's not occupied yet by any of the
predefined [RStudio shortcuts](https://support.rstudio.com/hc/en-us/articles/200711853-Keyboard-Shortcuts).
